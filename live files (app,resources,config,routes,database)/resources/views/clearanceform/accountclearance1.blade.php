                            ...
                            <div class="card">
                                    <div class="card-header">
                                        ...
                                        <!-- <form method="GET" action="{{ url()->current() }}">
                                            <select id="acctFilter" name="acct_status" style="float:right;">
                                                <option value="">All</option>
                                                <option value="N" {{ request()->get('acct_status', 'N') == 'N' ? 'selected' : '' }}>Pending</option>
                                                <option value="Y" {{ request()->get('acct_status') == 'Y' ? 'selected' : '' }}>Approved</option>
                                            </select>
                                        </form> -->
                                    </div>
                                </div>
...
// acct starts
                document.addEventListener("DOMContentLoaded", function () {
                    ...
                    function updateTotalDeductions() {
                        ...
                        $('#clearnsdetailsAcct').data('total-deductions', totalDeductions.toFixed(2));
                        
                        // Update totals (pass only total deductions for now)
                        updateTotals(totalDeductions, null);

                        // Update the total deductions field
                        $('#account-deductions').val(totalDeductions.toFixed(2));

                        // Log to debug
                        console.log("Updated Total Deductions:", totalDeductions.toFixed(2));
                    }
                    ...
                    function updateEarnings() {
                        ...
                        $('#clearnsdetailsAcct').data('total-earnings', totalEarnings.toFixed(2));

                        // Update totals (pass only total earnings for now)
                        updateTotals(null, totalEarnings);

                        // Update the total earnings field
                        $('#account-earnings').val(totalEarnings.toFixed(2));
                    ...
                    $(document).on('input', "input[name$='_Amt2']", function () {
                        updateEarnings();  // Update the total earnings when earning fields are modified
                            ...
                            function updateTotals(totalDeductions, totalEarnings) {
                                // Retrieve totalDeductions and totalEarnings from the data attributes if not passed
                                totalDeductions = totalDeductions !== null ? totalDeductions : parseFloat($('#clearnsdetailsAcct').data('total-deductions')) || 0;
                                totalEarnings = totalEarnings !== null ? totalEarnings : parseFloat($('#clearnsdetailsAcct').data('total-earnings')) || 0;

                                console.log("Total Deductions in updateTotals:", totalDeductions);  // Log total deductions
                                console.log("Total Earnings in updateTotals:", totalEarnings);  // Log total earnings

                                // Calculate total earnings and total deductions
                                var totalEarningsCalc = parseFloat(totalEarnings) || 0;  // Just use the total earnings directly
                                var totalDeductionsCalc = parseFloat(totalDeductions) || 0;  // Just use the total deductions directly
                                ...
                                // Retrieve the input elements
                                        ...
                                        // Calculate total earnings and total deductions
                                        var totalEarnings = itDeductionsValue + logisticsDeductionsValue + totalEarningsCalc + hrEarningInput;
                                        var totalDeductions = totalDeductionsCalc + logisticsEarningsValue + itEarningsValue + hrDeductionsInput;
            ...
            function populateAccountFormFields(nocData) {
                ...
                function recalculateTotals() {
                    ...
                    $('#account-earnings').val(totalEarning.toFixed(2));  // Format to 2 decimal places
                    $('#account-deductions').val(totalDeduct.toFixed(2));  // Format to 2 decimal places
                }
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                function calculateTotalAmount() {
                    ...
                    $('#total-amount-log').text("Total Amount: " + totalAmount.toFixed(2)); // Display total with two decimals
                }
            ...
            document.getElementById('add-more').addEventListener('click', function() {
                // Recalculate partyCount based on the number of existing parties in the DOM
                let partyCount = document.querySelectorAll('#parties-container .clformbox').length + 1;
                ...
                const partyHTML = `
                    <div class="clformbox" id="party-${partyCount}">
                        ...
                        <div class="clrecoveramt">
                            <input class="form-control" type="text" name="Parties_${partyCount}_Amt" placeholder="Enter recovery amount">
                        </div>
                        <div class="clreremarksbox">
                ...
                // Recalculate total amount after adding new party section
            ...
            document.getElementById('parties-container').addEventListener('click', function(event) {
                ...
                if (event.target && event.target.classList.contains('remove-party')) {
                    ...
                    // Recalculate total amount after removing a party section
                    calculateTotalAmount();
                }
            });
            ...
            $(document).ready(function() {
                ...
                success: function(response) {
                    ...
                    console.log(dealerNames);
                        ...
                        // Display the total amount
                        console.log(nocData.TotRepAmt);
                        $('#total-amount-log').text('Total Amount: ' + nocData.TotRepAmt);
                        ...
                        return total;
                            ...
                            function populateFormFields(nocData) {
                                ...
                                document.getElementById('it-tab').addEventListener('click', function() {
                                         $('#total-amount-it').text('Total Amount: ' + nocData.TotItAmt);
...
document.addEventListener("DOMContentLoaded", function () {
            ...
            $('#hr-earnings').val(nocData.TotEar);
                                ...
                                // Event listener for 'acct-tab' click
                                document.getElementById('acct-tab').addEventListener('click', function() {
                                    console.log(nocData.TotEar);
                                    console.log(nocData.TotDeduct);
                                                        ...
                                                        document.getElementById("hr-deductions").value = nocData.TotDeduct;  // Update the current value
                                                        document.getElementById("hr-deductions").setAttribute("value", nocData.TotDeduct);  // Update the value attribute
