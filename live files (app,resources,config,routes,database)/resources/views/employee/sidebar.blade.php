...
<aside class="sidebar-wrapper">
    ...
    <div class="side-menu-wrap active">
        <ul class="main-menu in">
            ...
            @if ($exists)
            <li>
                <a href="{{ route('teamleaveatt') }}" title="My Team">
                    <span class="icon-menu feather-icon text-center">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round"
            ...
            $pendingLedgerQueries = DB::table('hrm_employee_queryemp')
            ->where('QuerySubject', 'Ledger Confirmation')
            ->where(function ($query) use ($authEmployeeId) {
            $query->where('Level_1ID', $authEmployeeId)
            ->orWhere('AssignEmpId', $authEmployeeId);
            })
            ->where(function ($query) {
            $query->whereIn('QStatus', [0, 1])
            ->orWhereIn('Level_1QStatus', [0, 1]);
            })
            ->count();
            @endphp

            <li>
                <a href="{{ route('query') }}" title="Query" class="position-relative">
                    <span class="icon-menu feather-icon text-center">

                        {{-- Notification count bubble above the icon --}}
                        @if($pendingLedgerQueries > 0)
                        <span style="
                                            position: absolute;
                                            top: -6px;
                                            left: 55px;
                                            ...
                                            text-align: center;
                                        " title="Ledger Confirmation queries pending">
                            {{ $pendingLedgerQueries }}
                        </span>
                        @endif
            ...
            @if(Auth::user()->EmployeeID == '1763' || Auth::user()->EmployeeID == '169' || Auth::user()->EmployeeID == '1707' || Auth::user()->EmployeeID == '1369')
            <li>
                <a href="{{ route('expenses.ledger') }}" title="Expenses Ledger">
                    <span class="icon-menu feather-icon text-center">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round"
