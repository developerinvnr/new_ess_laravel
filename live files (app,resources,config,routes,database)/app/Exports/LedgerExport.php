...
class LedgerExport implements FromView
{
    ...
    protected $empCodes;
     ...
     public function __construct($type, $department = null, $search = null,$empCodes = [])
    {
        $this->type = $type;
               ...
               $data = DB::table('hrm_employee as e')
                        ...
                        ->when($this->search, function ($query) {
                            ...
                            $query->where(function ($q) use ($searchTerm) {
                                $q->whereRaw('LOWER(e.EmpCode) LIKE ?', [$searchTerm])
                                ->orWhereRaw('LOWER(e.Fname) LIKE ?', [$searchTerm])
                                ->orWhereRaw('LOWER(e.Sname) LIKE ?', [$searchTerm])
                                ->orWhereRaw('LOWER(e.Lname) LIKE ?', [$searchTerm]);
                            });
                        })
                        ->where('e.CompanyId', 1)
            ...
            $data = DB::table('hrm_employee as e')
              ...
              ->when($this->department, fn($q) => $q->where('d.department_name', $this->department))
                ->when($this->search, function ($query) {
                    ...
                    $query->where(function ($q) use ($searchTerm) {
                        $q->where('e.EmpCode', 'like', $searchTerm)
                        ->orWhereRaw("CONCAT_WS(' ', e.Fname, e.Sname, e.Lname) LIKE ?", [$searchTerm]);
                    });
                })
                        ->where('e.CompanyId', 1)
                        ->orderByRaw('CAST(e.EmpCode AS UNSIGNED) ASC')
                ...
                if ($this->type === 'queried' && !empty($ledgerQueryEmployeeIds)) {
                    $data = DB::table('hrm_employee as e')
                        ...
                        ->select('e.EmployeeID', 'e.EmpCode', DB::raw("CONCAT_WS(' ', e.Fname, e.Sname, e.Lname) as full_name"), 'd.department_name', 'g.EmailId_Vnr', 'q.QueryDT as QueryRaisedAt', 'q.Level_1QStatus as status')
