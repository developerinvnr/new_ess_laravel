...
class PmsController extends Controller
{
    ...
    public function saveRatingBreakdown(Request $request)
    {
        ...
        $effectiveHodId = (!empty($hodactualid) && $hodactualid !== "null") ? $hodactualid : $hoid;
            ...
            if (!$existing || $existing->HodSubmit_IncStatus != 2) {
                ...
                DB::table('hrm_employee_pms')
                    ...
                    ->update([
                        ...
                        'totgrosswithaddingest'     => $data['totgrosswithadding'],
      ...
      $query = DB::table('hrm_pms_workingsheet_inc')
        ...
        if (!empty($deptId) && $deptId !== "null") {
            $query->where('deptid', $deptId)
                ->where('hodid', !empty($hodactualid) && $hodactualid !== "null" ? $hodactualid : $hodid);
        } elseif (!empty($hodactualid) && $hodactualid !== "null") {
            $query->where('hodid', $hodactualid);
        }
    ...
    public function saveSingle(Request $request)
    {
        ...
        $variablepay  = preg_replace('/[^0-9.]/', '', $request->input('variablepay'));
        $totactctcwithcarpay  = preg_replace('/[^0-9.]/', '', $request->input('totactctcwithcarpay'));
        $totgrosswithadding  = preg_replace('/[^0-9.]/', '', $request->input('totgrosswithadding'));
        ...
        $dataToUpdate = [
            ...
            'Hod_ActualInc_Amt'          => $actualIncAmt,
            'Hod_ProRataInc_Amt'         => $proRataIncAmt,
            'variablepayest'            => $variablepay,
            'totactctcwithcarpayest'    => $totactctcwithcarpay,
            'totgrosswithaddingest'     => $totgrosswithadding,
    ...
    public function search_gross_ctc(Request $request)
    {
        // Get the employees data from the request
        $employees = $request->input('employees', []);

        // Collect unique values for designation, grade, and department
        $designationNames = collect($employees)->pluck('designation_name')->unique();
        $gradeNames = collect($employees)->pluck('grade_name')->unique();
        $departmentNames = collect($employees)->pluck('department_name')->unique();

        // Fetch corresponding IDs from the database for designations, grades, and departments
        $designationMap = DB::table('core_designation')
            ->whereIn('designation_name', $designationNames)
            ->pluck('id', 'designation_name');

        $gradeMap = DB::table('core_grades')
            ->whereIn('grade_name', $gradeNames)
            ->pluck('id', 'grade_name');

        $departmentMap = DB::table('core_departments')
            ->whereIn('department_name', $departmentNames)
            ->pluck('id', 'department_name');

        // Update the employees data with corresponding IDs
        $employees = array_map(function ($employee) use ($designationMap, $gradeMap, $departmentMap) {
            return array_merge($employee, [
                'department_id' => $departmentMap[$employee['department_name']] ?? null,
                'designation_id' => $designationMap[$employee['designation_name']] ?? null,
                'grade_id' => $gradeMap[$employee['grade_name']] ?? null,
            ]);
        }, $employees);

        // Prepare the calculated details array
        $calculatedDetails = [];
        ...
        foreach ($employees as $employee) {
            ...
            // Extract necessary fields from employee data
            $employee_id = $employee['employee_id'];
            $department_id = $employee['department_id'];
            $designation_id = $employee['designation_id'];
            $grade_id = $employee['grade_id'];
            $old_gross_salary = $employee['old_gross_salary'];
            $old_ctc = $employee['old_ctc'];
            $new_ctc = $employee['fixed_ctc'];  // This is the `new_ctc`

            // Fetch skill category (MW value)
            $skill_category = DB::table('hrm_deptgradedesig')
                ->where('DesigId', $designation_id)
                ->where('DepartmentId', $department_id)
                ->where(function ($query) use ($grade_id) {
                    $query->whereIn('GradeId', [$grade_id, 'GradeId_2', 'GradeId_3', 'GradeId_4', 'GradeId_5']);
                })
                ->where('CompanyId', 1)
                ->value('MW');

            // Determine bonus wage per month based on skill category
            $bonusWages = [
                1 => 13386,
                2 => 12606,
                3 => 11826,
            ...
            $bonus_per_month = $bonusWages[$skill_category] ?? 0;

            // Calculate bonus per month based on skill category
            $bonusM = 0;
            if ($bonus_per_month > 0) {
                $bonusTable = [1 => 0.20, 2 => 0.20, 3 => 0.0833];
                $bonusM = round($bonus_per_month * ($bonusTable[$skill_category] ?? 0));
            }

            // Calculate the difference between old and new CTC
            $diffCtc = $this->calculateCtcDifference($old_ctc, $new_ctc);

            // Adjustment ranges for gross salary increments
            $adjustmentRanges = [
                ['min' => 2000000, 'max' => 10000000, 'value' => 500000],
                ['min' => 1000000, 'max' => 2000000, 'value' => 100000],
                ['min' => 500000, 'max' => 1000000, 'value' => 10000],
                ['min' => 100000, 'max' => 500000, 'value' => 1000],
                ['min' => 10000, 'max' => 100000, 'value' => 500],
                ['min' => 5000, 'max' => 10000, 'value' => 50],
                ['min' => 1000, 'max' => 5000, 'value' => 20],
                ['min' => 500, 'max' => 1000, 'value' => 10],
                ['min' => 200, 'max' => 500, 'value' => 5],
                ['min' => 100, 'max' => 200, 'value' => 2],
                ['min' => 1, 'max' => 100, 'value' => 1]
            ];

            // Threshold to decide adjustment direction
            $ctcThresholds = [
                'min15' => $this->roundAmount($new_ctc - 15),
                'max10' => $this->roundAmount($new_ctc + 10)
            ];

            // Initial adjusted gross logic
            $grossToUse = $old_gross_salary;
            if ($old_ctc < $ctcThresholds['min15']) {
                $grossToUse = $this->calculateAdjustedGross($old_gross_salary, $diffCtc, $employee_id, $adjustmentRanges);
            } elseif ($old_ctc > $ctcThresholds['max10']) {
                $diffCtc = $this->roundAmount($old_ctc - $new_ctc);
                $grossToUse = $this->calculateAdjustedGross($old_gross_salary, $diffCtc, $employee_id, $adjustmentRanges, false);
            }

            // Recursively adjust gross until CTC matches
            $calculatedDetails[$employee_id] = $this->adjustGrossUntilCTCMatch(
                $grossToUse,
                $bonusM,
                $new_ctc,
                $employee_id,
        ...
        // Return calculated details as a response
    ...
    private function calculateCtcDifference($oldCtc, $newCtc): float
    {
        return $this->roundAmount(
            $oldCtc > $newCtc ? $newCtc : $newCtc - $oldCtc
        );
    }
    ...
    public function storeKRAEntries(Request $request)
    {
        ...
        $employeeId = Auth::user()->EmployeeID;
            ...
            if ($kraData->isEmpty()) {
                ...
                // Determine Calendar Year or Financial Year
                if ($joiningYear < 2025 && $yearId <= 13 && $CompanyId == '1') {
                    $startMonth = 1;
                    $yearType = 'CY';
                    $totalMonths = 12 - ($joiningMonth - 1);
                } elseif ($joiningYear < 2025 && $yearId <= 13 && ($CompanyId == '2' || $CompanyId == '3')) {
                    $startMonth = 4;
                    $yearType = 'FY';
                ...
                $activeQuarters = array_filter(array_keys($quarters), function ($q) use ($joiningMonth, $quarters) {
                    return !empty(array_intersect($quarters[$q], range($joiningMonth, 12))) || ($joiningMonth < 4 && $q === 'Q4');
                });

                $activeHalfYears = array_filter(array_keys($halfYears), function ($h) use ($joiningMonth, $halfYears) {
                    return !empty(array_intersect($halfYears[$h], range($joiningMonth, 12))) || ($joiningMonth < 4 && $h === 'H2');
                });
                ...
                // Determine year type and effective year
                $effectiveYear = DB::table('hrm_year')
                    ->where('PmsYearId', $yearId)
                    ->value('ToDate');
                $effectiveYear = $effectiveYear ? Carbon::parse($effectiveYear)->year : now()->year;

                if ($yearId <= 13 && $joiningYear <= 2024) {
                    $yearType = ($CompanyId == '1') ? 'CY' : 'FY';
                } else {
                    $yearType = 'FY';
                }

                // Consolidated period generation logic
                $kraSource = $subKraData ?? $subKraDatamain;
                if ($kraSource) {

                    $periods = [];

                    if ($kraSource->Period == 'Monthly') {
                        $serialNumber = 1;
                        $countRow = 12;
                        $allMonths = [];
                        $activeMonths = [];
                        ...
                        for ($i = 0; $i < $countRow; $i++) {
                            ...
                            if ($joiningYear < 2024) {
                                $isActive = true;
                            } elseif ($yearId == 14 && $joiningYear == 2024) {
                                $isActive = true;
                            } elseif ($yearType === 'CY') {
                                $isActive = $month >= $joiningMonth;
                            } else {
                        ...
                        $activeMonthCount = count($activeMonths);
                        $targetPerPeriod = $activeMonthCount > 0 ? round($kraSource->Target / $activeMonthCount, 2) : 0;
                        $weightagePerPeriod = $activeMonthCount > 0 ? round($kraSource->Weightage / $activeMonthCount, 2) : 0;
                    ...
                    elseif ($kraSource->Period == 'Quarter') {
                        $serialNumber = 1;
                        $isCalendarYear = ($yearType === 'CY');
                        $quarters = $isCalendarYear
                            ? [1 => [1, 2, 3], 2 => [4, 5, 6], 3 => [7, 8, 9], 4 => [10, 11, 12]]
                        ...
                        $activeQuarters = [];
                        foreach ([1, 2, 3, 4] as $q) {
                            if ($joiningYear < 2024 || ($yearId == 14 && $joiningYear == 2024) || $q >= $joiningQuarter) {
                                $activeQuarters[] = $q;
                            }
                        }

                        $activeQuarterCount = count($activeQuarters);
                        $targetPerQuarter = $activeQuarterCount > 0 ? round($kraSource->Target / $activeQuarterCount, 2) : 0;
                        $weightagePerQuarter = $activeQuarterCount > 0 ? round($kraSource->Weightage / $activeQuarterCount, 2) : 0;
                        ...
                        foreach ([1, 2, 3, 4] as $q) {
                    ...
                    else if ($kraSource->Period == '1/2 Annual') {
                            $serialNumber = 1;
                            $isCalendarYear = ($yearType === 'CY');
                            ...
                            // Determine active half-years
                            $activeHalves = [1, 2]; // Default to both half-years
                            if ($joiningYear < 2024) {
                                // All half-years active for employees who joined before 2024
                                $activeHalves = [1, 2];
                            } elseif ($yearId == 14 && $joiningYear == 2024) {
                                // For yearId 14 and joining in 2024, ignore joining date and include both half-years
                                $activeHalves = [1, 2];
                            } else {
                                // For other cases, check joining month
                                $joiningHalfYear = null;
                                foreach ($halfYears as $h => $months) {
                                    if (in_array($joiningMonth, $months)) {
                                        $joiningHalfYear = $h;
                                        break;
                            ...
                            $activeHalfCount = count($activeHalves);
                            $targetPerHalf = $activeHalfCount > 0 ? round($kraSource->Target / $activeHalfCount, 2) : 0;
                            $weightagePerHalf = $activeHalfCount > 0 ? round($kraSource->Weightage / $activeHalfCount, 2) : 0;
                ...
                // If no valid KRA or Sub-KRA data is found
                return response()->json(['success' => false, 'message' => 'No valid KRA or Sub-KRA data found.']);
            }

            // If kraData is not empty, return the fetched data
            return response()->json([
                'success' => true,
                'kraData' => $kraData,
                'subKraData' => $subKraData,
