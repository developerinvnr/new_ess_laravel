               ...
               //prorata = (518 / 360 * 12) + 3 ≈ 17.27 + 3 = 20.27 ✅
				...
				function formatDateToYMDLocal(date) {
						...
						if (dojnew <= '2023-06-30') {
							...
							const workingDays = calculateWorkingDays(dojDate, new Date('2024-12-31'));
							console.log('daysWorked else1234',workingDays);

							prorata = ((workingDays / 360) * finalActual) + threeMonthPortion;
							console.log(prorata);
						} else if (dojnew >= '2024-01-01' && dojnew <= '2024-09-30') {
							const workingDays = calculateWorkingDays(dojDate, new Date('2024-12-31'));
							console.log('daysWorked else12344545545',workingDays);
				...
				function recalculateRow(row, doj, forceUpdate = false) {
					...
					if (totalCTC > maxVCtc && maxVCtc > 0 && !triggeredByCorr) {
						...
						console.log('roundedProRataNew',roundedProRataNew);
						...
						console.log('ctc333',ctcEl.textContent);
						console.log('corr3333',corr);
						console.log('totalctcprocap3333',totalctcprocap);
						...
						const roundedProRataNewNoCap = Math.floor(prorata * 10) / 10;
						console.log('Sdsadasd',roundedProRataNewNoCap);
				...
				// function recalculateRow(row, doj, forceUpdate = false) {
				// 	clearTimeout(recalcTimer); // clear any previous timer
				...
				// 		let isCapping = false;
				// 		row.querySelector('.totctcnew').textContent = totalCTC.toFixed(2);
				// 		const accurateProrata = prorata;
				// 		const onloadcapcheck = (prevFixed + ((Math.floor(accurateProrata * 10) / 10) / 100) * prevFixed).toFixed(1);
				// 		console.log('✅ onloadcapcheckh:', onloadcapcheck);


				// 		if (totalCTC > maxVCtc && maxVCtc > 0 && !triggeredByCorr) {
				// 			isCapping = true;
				// 			row.classList.add('highlight-row');
				// 			const prorataFromWorkingDay = calculateProrata(row, doj);
				// 			console.log('✅ Using prorata from calculateWorkingDay:', prorataFromWorkingDay);

				// 			const accurateProrata = prorataFromWorkingDay;
				// 			console.log(
				// 				'✅ Using accurate prorata inside if:',
				// 				(Math.floor(parseFloat(accurateProrata) * 10) / 10).toFixed(1) / 100
				// 				);
				...
				// 			const actualnewinc = ((Math.floor(parseFloat(accurateProrata) * 10) / 10).toFixed(1) / 100) * maxctcannual;
				// 			const actualnewproposed = actualnewinc + prevFixed;
				// 			// console.log('actualnewproposed', actualnewproposed);
				// 			// console.log('actualnewinc', actualnewinc);
				// 			// console.log('prevFixed', prevFixed);
				// 			// console.log('maxctcannual', maxctcannual);



				// 			const proRatanew = ((actualnewproposed - prevFixed) / prevFixed) * 100;
				// 			const roundedProRataNew = Math.floor(proRatanew * 10) / 10; // One decimal place without converting to string
				...
				// 			// console.log('ctc', ctcEl.textContent);

				// 			// console.log('corr', corr);

				// 			// console.log('ctc+corrr', parseFloat(ctcEl.textContent) + corr);
				...
				// 			totalCtcEl.textContent = formatNumberround( parseFloat(ctcEl.textContent) + corr);
				...
				// 	let isCapping = false;
				// 	row.querySelector('.totctcnew').textContent = totalCTC.toFixed(2);
				// 	const accurateProrata = prorata; // Use this, not textContent
				// 	console.log('✅ Using accurate prorat1a:', accurateProrata);
				...
				// 		// Now use the prorata value from calculateWorkingDay for calculations inside the if
				// 		const accurateProrata = prorataFromWorkingDay; // Use the prorata fetched from the function
				// 		console.log('✅ Using accurate prorata inside if:', accurateProrata);
				...
				// 		console.log('proRatanew', proRatanew);
			...
			document.addEventListener('DOMContentLoaded', function () {
				...
				function recalculateRow(row, doj, forceUpdate = false) {
					...
					if (totalCTC > maxVCtc && maxVCtc > 0 && !triggeredByCorr) {
						...
						const totalctcprocap = parseFloat(ctcEl.textContent) + corr;
						console.log('ctc4444',ctcEl.textContent);
						console.log('corr44443',corr);
						console.log('totalctcprocap44443',totalctcprocap);
						const totalIncCap = totalctcprocap - prevFixed;
						...
						const roundedProRataNewNoCap = Math.floor(prorata * 10) / 10;
						console.log('roundedProRataNewNoCapdsdsdsd',roundedProRataNewNoCap);
				...
				// 		row.querySelector('.totctcnew').textContent = totalCTC.toFixed(2);
				// 		const accurateProrata = prorata;
				// 		console.log('✅ Using accurate prorat1aswsws:', accurateProrata);
				...
				// 			console.log('proRatanew', roundedProRataNew.toFixed(1));
				// 			console.log('THSI ONE', prevFixed + ((roundedProRataNew / 100) * prevFixed));
