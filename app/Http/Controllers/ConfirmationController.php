        ...
        public function store(Request $request)
{
    ...
    if ($request->submit_type == 'final') {
        ...
        if ($request->recommendation == "extendProbationRadio") {
            ...
            // Set confirmation date to 90 days from today if "extendProbationRadio" is selected
           $extendeddate = now()->addDays(90)->toDateString();
          // Retrieve the record for the employee from 'hrm_employee_general'
            $existingRecordgeneral = \DB::table('hrm_employee_general')->where('EmployeeID', $request->employeeId)->first();

            if ($existingRecordgeneral) {
                // If record exists, update the existing one
                \DB::table('hrm_employee_general')
                    ->where('EmployeeID', $request->employeeId)
                    ->update(['DateConfirmation' => $extendeddate]);  // Use an associative array for updating the column
            }
        }
